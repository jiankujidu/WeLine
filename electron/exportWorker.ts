...
process.env.WEFLOW_PROJECT_NAME = process.env.WEFLOW_PROJECT_NAME || 'WeLine'

async function run() {
  const [{ wcdbService }, { exportService }, { chatService }] = await Promise.all([
  ...
  chatService.setRuntimeConfig(runtimeCfg)
